	...
	if (isUser) {
		return (
			<div className="rounded-xl border border-border/80 bg-muted/40 px-4 py-3 shadow-[0_8px_24px_rgba(0,0,0,0.22)]">
				<div className="whitespace-pre-wrap text-xs font-medium leading-relaxed text-foreground">
					{content}
				</div>
			</div>
		);
	...
	return (
		<div className="flex gap-3">
			...
			<div className="min-w-0 flex-1">
				...
				{isThinking && (
					<div className="mb-2 inline-flex items-center gap-1 text-xs text-muted-foreground/90">
						<span>Thinking</span>
				...
				<div className="mt-1 px-0.5 text-[9px] text-muted-foreground">{formatTime(timestamp)}</div>
			</div>
		</div>
	);
};
