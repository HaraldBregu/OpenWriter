...
import type { FileEntryChangeEvent } from '../../shared/types';
...
export class FilesWatcherService implements Disposable {
  ...
  private static readonly LOG_SOURCE = 'FilesWatcherService';
  ...
  constructor(
    ...
    this.logger?.info(FilesWatcherService.LOG_SOURCE, 'Initialized');
  }
  ...
  async startWatching(workspacePath: string): Promise<void> {
    ...
    try {
      ...
      this.logger?.error(FilesWatcherService.LOG_SOURCE, 'Failed to create files directory', err);
      return;
    }
    ...
    try {
      this.watcher = chokidar.watch(filesDir, {
        ...
        ignored: (filePath: string) => {
          ...
          if (normalized === filesDir) return false;
      ...
      this.watcher
        ...
        .on('ready', () => {
          this.logger?.info(FilesWatcherService.LOG_SOURCE, `Watcher ready for: ${filesDir}`);
        });

      this.currentDirectory = filesDir;
    } catch (error) {
      this.logger?.error(FilesWatcherService.LOG_SOURCE, 'Failed to start watching', error);
      this.watcher = null;
  ...
  async stopWatching(): Promise<void> {
    ...
    try {
      ...
      this.logger?.error(FilesWatcherService.LOG_SOURCE, 'Error closing watcher', error);
    } finally {
      this.watcher = null;
      this.currentDirectory = null;
  ...
  destroy(): void {
    ...
    this.stopWatching().catch((error) => {
      this.logger?.error(FilesWatcherService.LOG_SOURCE, 'Error during destroy', error);
    });
  }
  ...
  private handleWorkspaceChange(newWorkspacePath: string | null): void {
    if (newWorkspacePath) {
      this.startWatching(newWorkspacePath).catch((error) => {
        this.logger?.error(
          FilesWatcherService.LOG_SOURCE,
      ...
      this.stopWatching().catch((error) => {
        this.logger?.error(FilesWatcherService.LOG_SOURCE, 'Failed to stop watcher', error);
      });
    }
  }

  private handleEvent(filePath: string, type: FileEntryChangeEvent['type']): void {
  ...
  private debouncedEmit(filePath: string, type: FileEntryChangeEvent['type']): void {
  ...
  private emitChangeEvent(filePath: string, type: FileEntryChangeEvent['type']): void {
    const fileId = path.basename(filePath);
    if (!fileId) return;

    const event: FileEntryChangeEvent = { type, fileId, filePath, timestamp: Date.now() };
    this.logger?.info(FilesWatcherService.LOG_SOURCE, `File ${type}: ${fileId}`);
    this.eventBus.broadcast('files:changed', event);
  }

  private handleWatcherError(error: unknown): void {
    const errorMessage = error instanceof Error ? error.message : String(error);
    this.logger?.error(FilesWatcherService.LOG_SOURCE, 'Watcher error', error);
    this.eventBus.broadcast('files:watcher-error', {
      error: errorMessage,
      timestamp: Date.now(),
    });
  }
