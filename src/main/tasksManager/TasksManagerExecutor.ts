/**
 * TasksManagerExecutor -- orchestrator for background task execution.
 *
 * Responsibilities:
 *  1. Accept task submissions (type + input) and queue them by priority.
 *  2. Look up the handler in TasksManagerHandlerRegistry.
...
import type { TasksManagerHandlerRegistry } from './TasksManagerHandlerRegistry'
...
export class TasksManagerExecutor implements Disposable {
  ...
  constructor(
    private readonly registry: TasksManagerHandlerRegistry,
  ...
  submit<TInput>(type: string, input: TInput, options?: TaskOptions): string {
    ...
    console.log(`[TasksManagerExecutor] Task ${taskId} queued (type="${type}", priority=${priority})`)
  ...
  cancel(taskId: string): boolean {
    ...
    console.log(`[TasksManagerExecutor] Cancelling task ${taskId}`)
  ...
  pause(taskId: string): boolean {
    ...
    console.log(`[TasksManagerExecutor] Task ${taskId} paused`)
    return true
  }
  ...
  resume(taskId: string): boolean {
    ...
    console.log(`[TasksManagerExecutor] Task ${taskId} resumed at position ${position}`)
  ...
  updatePriority(taskId: string, newPriority: TaskPriority): boolean {
    ...
    console.log(`[TasksManagerExecutor] Task ${taskId} priority updated to "${newPriority}"`)
  ...
  destroy(): void {
    console.log(
      `[TasksManagerExecutor] Destroying, aborting ${this.activeTasks.size} task(s)`
    )
    if (this.gcHandle) {
  ...
  private async executeTask(queued: QueuedTask): Promise<void> {
    ...
    if (timeoutMs !== undefined) {
      task.timeoutHandle = setTimeout(() => {
        console.log(`[TasksManagerExecutor] Task ${taskId} timed out after ${timeoutMs}ms`)
        controller.abort()
      }, timeoutMs)
    }
    ...
    console.log(`[TasksManagerExecutor] Task ${taskId} started (type="${type}")`)
    ...
    try {
      ...
      console.log(`[TasksManagerExecutor] Task ${taskId} completed in ${durationMs}ms`)
    } catch (err) {
      // Task may have been cancelled via cancel()
      if (!this.activeTasks.has(taskId)) return

      if (err instanceof Error && err.name === 'AbortError') {
        console.log(`[TasksManagerExecutor] Task ${taskId} aborted`)
        task.status = 'cancelled'
        task.completedAt = Date.now()
        ...
        console.error(`[TasksManagerExecutor] Task ${taskId} failed:`, err)
      ...
      console.log(
        `[TasksManagerExecutor] Task ${taskId} (${type}) finished. ` +
