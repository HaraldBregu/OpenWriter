...
const tabsAPI = {
  ...
  select: (id: number, fileType: TabType): Promise<void> =>
    ipcRenderer.invoke('tabs:select', id, fileType),
...
const menuAPI: IMenuAPI = {
  disableReferencesMenuItems: (items: string[]): Promise<void> =>
    ipcRenderer.invoke('menu:disableReferencesMenuItems', items),
  updateViewApparatusesMenuItems: (
    items: { id: string; title: string; visible: boolean }[]
  ): Promise<void> => ipcRenderer.invoke('menu:updateViewApparatusesMenuItems', items),
  setTocVisibility: (visibility: boolean): Promise<void> =>
    ipcRenderer.invoke('menu:setTocVisibility', visibility),
  setTocMenuItemsEnabled: (isEnable: boolean): Promise<void> =>
    ipcRenderer.invoke('menu:setTocMenuItemsEnabled', isEnable)
}
...
const systemAPI: ISystemAPI = {
  ...
  getSymbols: (fontName: string): Promise<CharacterSet> =>
    ipcRenderer.invoke('system:getSymbols', fontName),
  getConfiguredSpcialCharactersList: (): Promise<CharacterConfiguration[]> =>
    ipcRenderer.invoke('system:getConfiguredSpcialCharactersList'),
  showMessageBox: (message: string): Promise<void> =>
    ipcRenderer.invoke('system:showMessageBox', message)
}
...
const applicationAPI: IApplicationAPI = {
  ...
  toolbarAdditionalItems: (): Promise<string[]> =>
    ipcRenderer.invoke('application:toolbarAdditionalItems'),
...
const documentAPI: IDocumentAPI = {
  ...
  createTemplate: (template: unknown, name: string): Promise<void> =>
    ipcRenderer.invoke('document:createTemplate', template, name),
  ...
  setApparatuses: (apparatuses: DocumentApparatus[]): Promise<void> =>
    ipcRenderer.invoke('document:setApparatuses', apparatuses),
  setLayoutTemplate: (layoutTemplate: unknown): Promise<void> =>
    ipcRenderer.invoke('document:setLayoutTemplate', layoutTemplate),
  setPageSetup: (pageSetup: unknown): Promise<void> =>
    ipcRenderer.invoke('document:setPageSetup', pageSetup),
