 ...
 * The hook delegates to window.workspace.* namespace.
 */
...
describe('useFilesystem', () => {
	it('should initialize and load watched directories', async () => {
		(window.workspace.getWatched as jest.Mock).mockResolvedValue(['/watched/dir']);
	...
	describe('openFile', () => {
		it('should open a file and set currentFile', async () => {
			...
			(window.workspace.openFile as jest.Mock).mockResolvedValue(mockFile);
		...
		it('should not set currentFile when dialog is cancelled', async () => {
			(window.workspace.getWatched as jest.Mock).mockResolvedValue([]);
			(window.workspace.openFile as jest.Mock).mockResolvedValue(null);
		...
		it('should set error on failure', async () => {
			(window.workspace.getWatched as jest.Mock).mockResolvedValue([]);
			(window.workspace.openFile as jest.Mock).mockRejectedValue(new Error('open failed'));
	...
	describe('saveFile', () => {
		it('should save a file and return true on success', async () => {
			(window.workspace.getWatched as jest.Mock).mockResolvedValue([]);
			(window.workspace.saveFile as jest.Mock).mockResolvedValue({
				success: true,
			...
			expect(window.workspace.saveFile).toHaveBeenCalledWith('file.txt', 'content');
		});

		it('should return false on failure', async () => {
			(window.workspace.getWatched as jest.Mock).mockResolvedValue([]);
			(window.workspace.saveFile as jest.Mock).mockRejectedValue(new Error('save failed'));
	...
	describe('writeFile', () => {
		it('should write to a specific path', async () => {
			...
			expect(success).toBe(true);
			expect(window.workspace.writeFile).toHaveBeenCalledWith('/a.txt', 'data');
		});
	});
	...
	describe('clearFile', () => {
		it('should reset currentFile to null', async () => {
			(window.workspace.getWatched as jest.Mock).mockResolvedValue([]);
			(window.workspace.openFile as jest.Mock).mockResolvedValue({
				filePath: '/x',
				fileName: 'x',
				content: 'x',
				size: 1,
	...
	it('should clean up fs event listener on unmount', async () => {
		...
		(window.workspace.getWatched as jest.Mock).mockResolvedValue([]);
		(window.workspace.onWatchEvent as jest.Mock).mockReturnValue(unsubscribe);

		const { unmount } = renderHook(() => useFilesystem());

		await waitFor(() => {
			expect(window.workspace.onWatchEvent).toHaveBeenCalled();
		});
