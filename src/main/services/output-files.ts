...
export interface ContentBlock {
  /** UUID matching the .md filename (without extension) */
  name: string
  /** Raw markdown text for this block */
  content: string
  /** Block filetype — currently always "markdown" */
  filetype: 'markdown'
  /** Block role / purpose tag — currently always "content" */
  type: 'content'
...
/**
 * Per-block descriptor stored inside the `content` array of config.json.
 * Does NOT include the actual markdown text — that lives in `<name>.md`.
 */
...
export interface OutputFileMetadata {
  ...
  /**
   * Ordered list of content block descriptors.
   * Each descriptor maps 1:1 to a `<name>.md` file in this folder.
   */
  content: ContentBlockDescriptor[]
}
...
export interface OutputFile {
  ...
  /** Ordered content blocks, each hydrated from its <name>.md file */
  blocks: ContentBlock[]
...
export interface SaveOutputFileInput {
  type: OutputType
  /**
   * Content blocks to write.
   * Pass a single-element array if you only have one block of markdown.
   */
  blocks: Array<{
    /** Stable UUID for this block. Generated by the caller. */
    name: string
    content: string
    filetype?: 'markdown'
    type?: 'content'
  }>
  metadata: Omit<OutputFileMetadata, 'createdAt' | 'updatedAt' | 'content'>
}
...
export interface UpdateOutputFileInput {
  /** Content blocks. Replaces all existing blocks for this entry. */
  blocks: Array<{
    name: string
    content: string
    /** If omitted, createdAt is preserved from the existing descriptor (or set to now for new blocks). */
    createdAt?: string
    filetype?: 'markdown'
    type?: 'content'
  }>
...
export class OutputFilesService implements Disposable {
   ...
   *   output/<type>/        depth 1
   *   output/<type>/<date>/ depth 2  <- date folders
   *   output/<type>/<date>/<file>    depth 3  <- config.json + block .md files
   */
