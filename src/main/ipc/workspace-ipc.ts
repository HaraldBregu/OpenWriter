...
export class WorkspaceIpc implements IpcModule {
	...
	register(container: ServiceContainer, eventBus: EventBus): void {
		...
		ipcMain.handle(
			WorkspaceChannels.insertResourcesImages,
			wrapIpcHandler(async (event: IpcMainInvokeEvent, extensions?: string[]) => {
				const hasFilter = extensions && extensions.length > 0;
				const filters: FileFilter[] = hasFilter
					? [
				...
				const workspaceService = ctx.getService<WorkspaceService>('workspace', container);
				const imagesService = ctx.getService<ImagesService>('imagesService', container);
				const imagesWatcher = ctx.container.has('imagesWatcher')
					? ctx.getService<ImagesWatcherService>('imagesWatcher', container)
					: null;
				...
				const markWritten = imagesWatcher
					? (destPath: string) => imagesWatcher.markFileAsWritten(destPath)
					: undefined;

				return imagesService.insertImages(currentPath, result.filePaths, markWritten);
			}, WorkspaceChannels.insertResourcesImages)
		);

		ipcMain.handle(
			WorkspaceChannels.deleteResourcesImageEntry,
			wrapIpcHandler(async (event: IpcMainInvokeEvent, id: string) => {
				const ctx = getWindowContext(event, container);
				const workspaceService = ctx.getService<WorkspaceService>('workspace', container);
				const imagesService = ctx.getService<ImagesService>('imagesService', container);
				...
				const markWritten = imagesWatcher
					? (filePath: string) => imagesWatcher.markFileAsWritten(filePath)
					: undefined;

				await imagesService.deleteImage(currentPath, id, markWritten);
			}, WorkspaceChannels.deleteResourcesImageEntry)
		);
