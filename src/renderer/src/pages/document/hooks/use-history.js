...
import { saveHistorySnapshot, listHistoryEntries, loadHistoryEntry, readLatestHistoryEntry, deleteHistoryEntries, MAX_HISTORY_ENTRIES, isHistoryEntryFilePath, } from '../services/history-service';
const SNAPSHOT_DEBOUNCE_MS = 1500;
const RESTORE_COOLDOWN_MS = 200;
export function useHistory({ documentId, content, title, loaded, onRestore, }) {
    ...
    const reloadEntries = useCallback(async () => {
        ...
        try {
            ...
            setCurrentEntryId((previousEntryId) => previousEntryId && nextEntries.some((entry) => entry.id === previousEntryId)
                ? previousEntryId
                : null);
        }
        catch {
    ...
    useEffect(() => {
        ...
        const unsubscribe = window.workspace.onOutputFileChange((event) => {
            if (event.outputType !== 'documents' || event.fileId !== documentId)
                return;
            if (!isHistoryEntryFilePath(event.filePath))
                return;
            void reloadEntries();
        });
        return unsubscribe;
    ...
    const debouncedSnapshot = useMemo(() => debounce(async () => {
        if (!docPath || isRestoringRef.current)
            return;
        const { content: c, title: t } = stateRef.current;
        if (c.trim() === '')
            return;
        const last = lastSnapshotRef.current;
        if (last && last.content === c && last.title === t)
            return;
        try {
    ...
    }, SNAPSHOT_DEBOUNCE_MS, { leading: false, trailing: true }), [docPath]);
    // Trigger snapshot on content/title changes
    useEffect(() => {
        if (!loaded || isRestoringRef.current)
            return;
        debouncedSnapshot();
    ...
    useEffect(() => {
        if (!loaded || isRestoringRef.current || currentEntryId === null)
            return;
