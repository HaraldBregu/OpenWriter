...
import { Heading1, Heading2, Heading3, Heading4, Heading5, Heading6, Type, List, ListOrdered, Minus, ImagePlus, Images as ImagesIcon, FileText, Video, Music, } from 'lucide-react';
...
import { OptionMenuPlugin, } from '../plugins/option-menu-plugin';
...
function toRelativePath(fromDir, toFile) {
...
export function OptionMenu() {
    ...
    const [imagesAnchorEl, setImagesAnchorEl] = useState(null);
    const [contentsAnchorEl, setContentsAnchorEl] = useState(null);
    const menuControlsRef = useRef({
        forceHide: () => undefined,
        dismiss: () => undefined,
    });
    ...
    const runHeading = useCallback((level) => {
    ...
    const runImageFromWorkspace = useCallback((image) => {
        const ctx = deleteSlash();
        if (!ctx)
            return;
        const storage = editor.storage;
        const basePath = storage.image?.documentBasePath ?? null;
        const src = basePath
    ...
    const runContentFromWorkspace = useCallback(async (content) => {
        const ctx = deleteSlash();
        if (!ctx)
            return;
        editor
    ...
    const imagesItemIndex = useMemo(() => flatItems.findIndex((it) => it.id === IMAGES_ITEM_ID), [flatItems]);
    const contentItemIndex = useMemo(() => flatItems.findIndex((it) => it.id === CONTENT_ITEM_ID), [flatItems]);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (!container)
            return;
        const target = container.querySelector(`[data-item-index="${selectedIndex}"]`);
        if (!target)
            return;
    ...
    const runByIndex = useCallback((idx) => {
    ...
    const onKeyEvent = useCallback((event) => {
        ...
        if (inImagesSubmenu) {
            ...
            if (event.key === 'ArrowRight' || event.key === 'ArrowDown') {
                ...
                if (len > 0)
                    setImageSelectedIndex((p) => Math.min(len - 1, p + 1));
                return true;
            }
        ...
        if (inContentsSubmenu) {
            ...
            if (event.key === 'ArrowDown') {
                event.preventDefault();
                if (len > 0)
                    setContentSelectedIndex((p) => Math.min(len - 1, p + 1));
                return true;
            }
            if (event.key === 'ArrowUp') {
                event.preventDefault();
    ...
    const handlePluginUpdate = useCallback((state) => {
    ...
    if (!isMounted)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("div", { ref: refs.setFloating, style: floatingStyles, className: "z-50", children: _jsx("div", { style: transitionStyles, className: "will-change-transform", children: _jsxs(Card, { size: "sm", className: cn('w-[260px] gap-0! py-0! m-0! text-left overflow-visible!', 'shadow-[0_0_20px_0_rgba(0,0,0,0.12)]! dark:shadow-[0_0_24px_0_rgba(0,0,0,0.55)]!'), children: [_jsx(CardHeader, { className: "px-3 py-2 border-b border-border/60 gap-0", children: _jsx(CardTitle, { className: "text-xs font-medium text-muted-foreground leading-none", children: "Blocks" }) }), _jsx(CardContent, { ref: scrollContainerRef, className: "flex flex-col gap-1 p-1! max-h-[340px] overflow-y-auto", children: sections.map((section, sIdx) => {
                                    const baseOffset = sections
                                        .slice(0, sIdx)
                                        .reduce((acc, s) => acc + s.items.length, 0);
                                    return (_jsxs("div", { className: "flex flex-col", children: [_jsx("div", { className: "px-2 pt-2 pb-1 text-xs text-muted-foreground", children: section.label }), section.items.map((item, iIdx) => {
                                                ...
                                                const anchorSetter = isImagesItem
                                                    ...
                                                    : isContentItem
                                                        ? setContentsAnchorEl
                                                        : undefined;
                                                return (_jsx("div", { ref: anchorSetter, children: _jsxs(Button, { "data-item-index": flatIdx, variant: isSelected ? 'secondary' : 'ghost', disabled: item.disabled, className: "w-full justify-start", onMouseEnter: () => {
                                                            if (!item.disabled)
                                                            ...
                                                            item.run?.();
                                                        }, children: [_jsx(Icon, {}), _jsx("span", { className: "truncate", children: item.label })] }) }, item.id));
                                            })] }, section.id));
                                }) }), _jsx(CardFooter, { className: "p-1! border-t border-border/60 bg-transparent", children: _jsxs(Button, { variant: "ghost", size: "sm", className: "w-full justify-between", onMouseDown: (e) => {
                                        e.preventDefault();
                                        handleClose();
                                    }, children: [_jsx("span", { children: "Close menu" }), _jsx("kbd", { className: "text-[10px] text-muted-foreground rounded border border-border/70 bg-muted/40 px-1.5 py-0.5 font-mono", children: "esc" })] }) })] }) }) }), _jsx(ImagesMenu, { open: imagesMenuOpen, anchor: imagesAnchorEl, images: images, selectedIndex: imageSelectedIndex, onSelectIndex: setImageSelectedIndex, onPick: runImageFromWorkspace, onMouseEnter: () => {
                    if (imagesItemIndex >= 0)
                        setSelectedIndex(imagesItemIndex);
                } }), _jsx(ContentsMenu, { open: contentsMenuOpen, anchor: contentsAnchorEl, contents: contents, selectedIndex: contentSelectedIndex, onSelectIndex: setContentSelectedIndex, onPick: runContentFromWorkspace, onMouseEnter: () => {
                    if (contentItemIndex >= 0)
                        setSelectedIndex(contentItemIndex);
                } })] }));
}
