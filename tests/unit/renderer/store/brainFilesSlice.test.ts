...
describe('personalityFilesSlice', () => {
	...
	describe('savePersonalityFile', () => {
		it('should record lastSaved on successful save', async () => {
			mockPersonalitySave.mockResolvedValue({
				id: '123',
				path: '/workspace/test',
				savedAt: Date.now(),
			});
			...
			await (store.dispatch as Function)(
				savePersonalityFile({
					...
					content: 'Hello',
					metadata: { title: 'Test', provider: 'openai', model: 'gpt-4' },
				})
			);
		...
		it('should set error message on save failure', async () => {
			...
			const state = (
				store.getState() as { personalityFiles: { error: string | null; lastSaved: null } }
			).personalityFiles;
		...
		it('should set loading=true while save is in-flight and loading=false on completion', async () => {
			...
			mockPersonalitySave.mockReturnValue(
				new Promise((res) => {
					resolvePromise = res;
				})
			);
			...
			expect(
				(store.getState() as { personalityFiles: { loading: boolean } }).personalityFiles.loading
			).toBe(true);
	...
	describe('loadPersonalityFiles', () => {
		it('should populate section buckets from the loaded files', async () => {
			...
			const state = (
				store.getState() as {
					personalityFiles: { files: Record<string, unknown[]>; loading: boolean; error: null };
				}
			).personalityFiles;
		...
		it('should set error message on load failure', async () => {
			...
			const state = (
				store.getState() as { personalityFiles: { error: string | null; loading: boolean } }
			).personalityFiles;
		...
		it('should sort files within each section by savedAt descending', async () => {
			...
			const files = (
				store.getState() as { personalityFiles: { files: Record<string, Array<{ id: string }>> } }
			).personalityFiles.files['emotional-depth'];
	...
	describe('deletePersonalityFile', () => {
		it('should remove the file from its section bucket on successful delete', async () => {
			...
			expect(
				(store.getState() as { personalityFiles: { files: Record<string, unknown[]> } })
					.personalityFiles.files['emotional-depth']
			).toHaveLength(1);

			mockPersonalityDelete.mockResolvedValue(undefined);
			await (store.dispatch as Function)(
				deletePersonalityFile({ id: '123', sectionId: SECTION_ID })
			);
		...
		it('should set error on delete failure', async () => {
			...
			expect(
				(store.getState() as { personalityFiles: { error: string | null } }).personalityFiles.error
			).toBe('Delete failed');
		});
	});
	...
	describe('reducers', () => {
		...
		it('clearLastSaved should reset lastSaved', async () => {
			...
			expect(
				(store.getState() as { personalityFiles: { lastSaved: unknown } }).personalityFiles
					.lastSaved
			).not.toBeNull();
	...
	describe('selectors', () => {
		it('selectAllPersonalityFiles should return the files map', () => {
			...
			const files = selectAllPersonalityFiles(
				state as Parameters<typeof selectAllPersonalityFiles>[0]
			);
		...
		it('selectPersonalityFilesLoading should return loading state', () => {
			const state = store.getState();
			expect(
				selectPersonalityFilesLoading(state as Parameters<typeof selectPersonalityFilesLoading>[0])
			).toBe(false);
		});
		...
		it('selectPersonalityFilesError should return null initially', () => {
			const state = store.getState();
			expect(
				selectPersonalityFilesError(state as Parameters<typeof selectPersonalityFilesError>[0])
			).toBeNull();
		});
		...
		it('selectTotalPersonalityFilesCount should return 0 initially', () => {
			...
			expect(
				selectTotalPersonalityFilesCount(
					state as Parameters<typeof selectTotalPersonalityFilesCount>[0]
				)
			).toBe(0);
		});
