...
function PageSidebarLayoutContainer({ className, style, children, ...props }) {
    return (_jsx("div", { "data-slot": "sidebar-wrapper", style: {
...
function PageSidebarLayout({ side = 'left', variant = 'sidebar', collapsible = 'offcanvas', className, children, dir, ...props }) {
    ...
    if (isMobile) {
        return (_jsx(Sheet, { open: state.sidebarOpenMobile, onOpenChange: (open) => dispatch({ type: 'SIDEBAR_OPEN_MOBILE_SET', open }), ...props, children: _jsxs(SheetContent, { dir: dir, "data-sidebar": "sidebar", "data-slot": "sidebar", "data-mobile": "true", className: "w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden", style: {
    ...
    return (_jsxs("div", { className: "group peer hidden text-sidebar-foreground md:block", "data-state": sidebarLayoutState, "data-collapsible": sidebarLayoutState === 'collapsed' ? collapsible : '', "data-variant": variant, "data-side": side, "data-slot": "sidebar", children: [_jsx("div", { "data-slot": "sidebar-gap", className: cn('relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-linear', 'group-data-[collapsible=offcanvas]:w-0', 'group-data-[side=right]:rotate-180', variant === 'floating' || variant === 'inset'
...
function PageSidebarLayoutTrigger({ className, onClick, ...props }) {
    ...
    return (_jsxs(Button, { "data-sidebar": "trigger", "data-slot": "sidebar-trigger", variant: "ghost", size: "icon-sm", className: cn(className), onClick: (event) => {
            onClick?.(event);
            toggleSidebar();
        }, ...props, children: [_jsx(PanelLeftIcon, { className: "cn-rtl-flip" }), _jsx("span", { className: "sr-only", children: "Toggle Sidebar" })] }));
}
function PageSidebarLayoutRail({ className, ...props }) {
    const { toggleSidebar } = usePageContext();
...
function PageSidebarLayoutContent({ className, ...props }) {
...
function PageSidebarLayoutGroup({ className, ...props }) {
...
function PageSidebarLayoutGroupLabel({ className, render, ...props }) {
    return useRender({
        defaultTagName: 'div',
        props: mergeProps({
...
function PageSidebarLayoutGroupAction({ className, render, ...props }) {
    return useRender({
...
function PageSidebarLayoutGroupContent({ className, ...props }) {
