...
export function buildGraph(
	...
	const nodes: Record<AssistantNodeName, AssistantSpecialistAgent> = {
		[ASSISTANT_NODE.ROUTE_QUESTION]: assistantNodeDefinitions[ASSISTANT_NODE.ROUTE_QUESTION].create(
			m[ASSISTANT_NODE.ROUTE_QUESTION]
		),
		[ASSISTANT_NODE.GENERATE_DIRECT_ANSWER]: assistantNodeDefinitions[
			ASSISTANT_NODE.GENERATE_DIRECT_ANSWER
		].create(m[ASSISTANT_NODE.GENERATE_DIRECT_ANSWER]),
		[ASSISTANT_NODE.RETRIEVE_DOCUMENTS]: assistantNodeDefinitions[
			ASSISTANT_NODE.RETRIEVE_DOCUMENTS
		].create(m[ASSISTANT_NODE.RETRIEVE_DOCUMENTS]),
		[ASSISTANT_NODE.GRADE_DOCUMENTS]: assistantNodeDefinitions[
			ASSISTANT_NODE.GRADE_DOCUMENTS
		].create(m[ASSISTANT_NODE.GRADE_DOCUMENTS]),
		[ASSISTANT_NODE.REWRITE_QUERY]: assistantNodeDefinitions[ASSISTANT_NODE.REWRITE_QUERY].create(
			m[ASSISTANT_NODE.REWRITE_QUERY]
		),
		[ASSISTANT_NODE.RETURN_FALLBACK_RESPONSE]: assistantNodeDefinitions[
			ASSISTANT_NODE.RETURN_FALLBACK_RESPONSE
		].create(m[ASSISTANT_NODE.RETURN_FALLBACK_RESPONSE]),
		[ASSISTANT_NODE.GENERATE_ANSWER]: assistantNodeDefinitions[
			ASSISTANT_NODE.GENERATE_ANSWER
		].create(m[ASSISTANT_NODE.GENERATE_ANSWER]),
	};
