...
describe('PersonalityFilesService', () => {
	...
	describe('save', () => {
		it('should write config.json and DATA.md inside a date-named folder', async () => {
			...
			const dataMdCall = (mockFs.writeFile as jest.Mock).mock.calls.find((c: unknown[]) =>
				(c[0] as string).endsWith('DATA.md')
			);
			expect(dataMdCall).toBeDefined();
			expect(dataMdCall![1]).toBe('Personality content');

			const configCall = (mockFs.writeFile as jest.Mock).mock.calls.find((c: unknown[]) =>
	...
	describe('delete', () => {
		...
		it('should not throw when neither folder nor legacy file exist (ENOENT)', async () => {
			...
			await expect(service.delete(SECTION, 'missing-folder')).resolves.not.toThrow();
		});
		...
		it('should throw when deletion fails with a non-ENOENT error', async () => {
			...
			await expect(service.delete(SECTION, 'locked-folder')).rejects.toThrow('Permission denied');
		});
	});
	...
	describe('missing workspace', () => {
		it('should throw from loadOne when no workspace is selected', async () => {
			...
			await expect(service.loadOne(SECTION, 'some-id')).rejects.toThrow(/No workspace/);
		});
