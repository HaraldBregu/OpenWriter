...
describe('EchoAgent', () => {
	...
	describe('run', () => {
		it('should emit thinking event first', async () => {
			...
			const generator = agent.run({ prompt: 'test' }, 'run-1', controller.signal);
		...
		it('should echo prompt word by word with spaces', async () => {
			...
			const generator = agent.run({ prompt: 'hello world test' }, 'run-1', controller.signal);
		...
		it('should handle single word prompt', async () => {
			...
			const generator = agent.run({ prompt: 'word' }, 'run-1', controller.signal);
		...
		it('should handle empty prompt', async () => {
			...
			const generator = agent.run({ prompt: '' }, 'run-1', controller.signal);
		...
		it('should handle prompt with multiple spaces', async () => {
			...
			const generator = agent.run({ prompt: 'hello    world' }, 'run-1', controller.signal);
		...
		it('should use provided runId in all events', async () => {
			...
			const generator = agent.run({ prompt: 'test' }, 'custom-run-id', controller.signal);
		...
		it('should include delays between tokens', async () => {
			...
			const generator = agent.run({ prompt: 'one two three' }, 'run-1', controller.signal);
