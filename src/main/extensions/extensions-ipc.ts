...
export class ExtensionsIpc implements IpcModule {
	...
	register(container: ServiceContainer, _eventBus: EventBus): void {
		...
		ipcMain.handle(
			...
			wrapIpcHandler((event, documentId: string) => {
				const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
				return manager.getDocPanels(windowId, documentId);
			}, ExtensionChannels.getDocPanels)
		);
		...
		ipcMain.handle(
			...
			wrapIpcHandler((event, panelId: string, documentId: string) => {
				const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
				return manager.getDocPanelContent(panelId, documentId, windowId, 'open');
			}, ExtensionChannels.getDocPanelContent)
		);
