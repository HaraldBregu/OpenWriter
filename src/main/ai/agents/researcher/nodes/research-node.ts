 ...
 * intent and the planned research angles as context. This is a non-streamed
 * invoke call — the resulting knowledge block feeds directly into the compose
 * node which produces the user-visible streamed response.
 */
...
const SYSTEM_PROMPT =
	...
	'about the topic given the user query, classified intent, and research angles. ' +
	'Cover each research angle thoroughly. Be factual, thorough, and well-organised. ' +
	'This synthesis will be used as the knowledge foundation for a final response.';

function buildHumanMessage(prompt: string, intent: string, plan: string[]): string {
...
export async function researchNode(
	...
	const messages = [
		...
		new HumanMessage(buildHumanMessage(state.prompt, state.intent, state.plan)),
	];
