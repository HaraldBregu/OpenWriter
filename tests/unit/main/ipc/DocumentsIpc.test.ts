...
describe('DocumentsIpc', () => {
	...
	describe('documents:load-all handler', () => {
		...
		it('should return metadata for files found in the documents directory', async () => {
			...
			mockFs.readdir.mockResolvedValueOnce(['report.txt'] as unknown as Awaited<
		...
		it('should skip hidden files (names starting with ".")', async () => {
			...
			mockFs.readdir.mockResolvedValueOnce(['.DS_Store', 'visible.txt'] as unknown as Awaited<
				ReturnType<typeof fs.readdir>
			>);
		...
		it('should skip directory entries inside the documents folder', async () => {
			mockFs.access.mockResolvedValueOnce(undefined);
			mockFs.readdir.mockResolvedValueOnce(['subdir', 'file.md'] as unknown as Awaited<
				ReturnType<typeof fs.readdir>
			>);
			mockFs.stat
				.mockResolvedValueOnce({
					...
					mtimeMs: 0,
					isDirectory: () => true,
				} as unknown as Awaited<ReturnType<typeof fs.stat>>) // subdir
				.mockResolvedValueOnce({
					size: 200,
					mtimeMs: 2000,
					isDirectory: () => false,
				} as unknown as Awaited<ReturnType<typeof fs.stat>>); // file.md
