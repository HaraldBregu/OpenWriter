...
const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    <ToolbarContainer className={viewToolbar ? '' : 'hidden'}>
      <div ref={toolbarContainer} onContextMenu={handleContextMenu} className={cn("relative w-full max-w-full flex justify-between space-x-2 bg-white overflow-hidden")}>
        <div
          style={{
            position: "absolute",
        ...
        <div className="flex items-center space-x-2 flex-nowrap">
          ...
          >
            ...
            <CustomSelect
              ...
              showSeparators={true}
              items={sectionTypes.map(type => ({
                value: type.value.toString(),
                label: <span>{t(type.label)}</span>
              }))}
            />
          </div>
          {/* FONT FAMILY SELECT */}
          ...
          >
            ...
            <Button
              ...
              icon={<Siglum intent='secondary' variant='tonal' size='small' />}
              onClick={() => null}
            ...
            }
            {
              ...
              <Button
                ...
                size="iconSm"
                tabIndex={17}
                tooltip={t('toolbar.readingSeparator')}
                icon={<ReadingSeperator intent='secondary' variant='tonal' size='small' />}
                onClick={() => {
                  // TODO: add toggle citation
                  // activeEditor?.chain().focus().toggleBlockquote().run()
                }}
                aria-label={t('toolbar.readingSeparator')}
                className="border-none shadow-none hover:bg-grey-80 focus-visible:ring-0 focus-visible:ring-offset-0"
              />
            }
          </div>
