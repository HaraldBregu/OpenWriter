...
const VALID_BLOCKS = [{ name: BLOCK_ID, content: '# Hello World' }];
...
describe('OutputFilesService', () => {
	...
	describe('save()', () => {
		...
		it('should throw for invalid output type', async () => {
			await expect(service.save({ ...VALID_INPUT, type: 'invalid' as any })).rejects.toThrow(
				'Invalid output type'
			);
		});

		it('should throw when blocks array is empty', async () => {
			await expect(service.save({ ...VALID_INPUT, blocks: [] })).rejects.toThrow(
		...
		it('should write valid JSON to config.json including a content descriptor array', async () => {
			...
			const configWriteCall = (mockWriteFile as jest.Mock).mock.calls.find((c: unknown[]) =>
				(c[0] as string).endsWith('config.json')
			);
		...
		it('should write the block content to a <name>.md file', async () => {
			await service.save(VALID_INPUT);
			const blockWriteCall = (mockWriteFile as jest.Mock).mock.calls.find((c: unknown[]) =>
				(c[0] as string).endsWith(`${BLOCK_ID}.md`)
			);
	...
	describe('Legacy DATA.md migration', () => {
		it('should migrate a legacy folder (no content array) on loadFolder', async () => {
			...
			const configWrite = (mockWriteFile as jest.Mock).mock.calls.find((c: unknown[]) =>
				(c[0] as string).endsWith('config.json')
			);
	...
	describe('update()', () => {
		...
		it('should throw when no workspace is selected', async () => {
			mockWorkspace.getCurrent.mockReturnValue(null);
			await expect(service.update('posts', DATE_FOLDER, VALID_UPDATE_INPUT)).rejects.toThrow(
		...
		it('should delete removed block files when blocks are removed', async () => {
			...
			expect(mockUnlink).toHaveBeenCalledWith(expect.stringContaining('block-to-remove.md'));
		});
	});
	...
	describe('delete()', () => {
		...
		it('should call fs.rm recursively when the folder exists', async () => {
			...
			expect(mockRm).toHaveBeenCalledWith(expect.stringContaining(DATE_FOLDER), {
				recursive: true,
			});
		});
		...
		it('should throw for invalid output type', async () => {
			await expect(service.delete('notes' as any, DATE_FOLDER)).rejects.toThrow(
				'Invalid output type'
			);
		});
	});
