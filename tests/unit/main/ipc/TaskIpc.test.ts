...
describe('TaskIpc', () => {
  ...
  describe('task:submit handler', () => {
    it('should submit a task and return a taskId', async () => {
      ...
      const result = (await handler(MOCK_EVENT, payload)) as {
        success: boolean;
        data: { taskId: string };
      };
    ...
    it('should return error response when executor.submit throws', async () => {
      ...
      const result = (await handler(MOCK_EVENT, { type: 'bad', input: {} })) as {
        success: boolean;
      };
  ...
  describe('task:cancel handler', () => {
    it('should return true when the task is successfully cancelled', async () => {
      ...
      const result = (await handler(MOCK_EVENT, 'task-abc-123')) as {
        success: boolean;
        data: boolean;
      };
      expect(result.success).toBe(true);
    ...
    it('should return false when the task is not found', async () => {
      ...
      const result = (await handler(MOCK_EVENT, 'nonexistent-task')) as {
        success: boolean;
        data: boolean;
      };
      expect(result.success).toBe(true);
  ...
  describe('task:list handler', () => {
    ...
    it('should return only serializable TaskInfo fields', async () => {
      ...
      const result = (await handler(MOCK_EVENT)) as {
        success: boolean;
