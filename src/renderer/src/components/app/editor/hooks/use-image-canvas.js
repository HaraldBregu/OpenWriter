...
function applyConvolutionFilter(imageData, kernel, divisor = 16) {
...
function applyBlurFilter(imageData, iterations = 1) {
...
function applyBrightnessFilter(imageData, amount = 30) {
    const { data } = imageData;
    const output = new Uint8ClampedArray(data.length);
    for (let i = 0; i < data.length; i += 4) {
...
function applySaturationFilter(imageData, amount = 1.3) {
    const { data } = imageData;
    const output = new Uint8ClampedArray(data.length);
    for (let i = 0; i < data.length; i += 4) {
        const r = data[i];
...
const EFFECT_DEFAULT_INTENSITIES = {
    blur: 2,
    brightness: 40,
    saturation: 1.5,
};
...
function drawRotatedImage(ctx, img, rotationDeg) {
...
// Hook
// ---------------------------------------------------------------------------
export function useImageCanvas(src) {
    ...
    // Snapshot helpers
    // ---------------------------------------------------------------------------
    const pushSnapshot = useCallback((rotation) => {
    ...
    // Rotation
    // ---------------------------------------------------------------------------
    const applyRotation = useCallback((direction) => {
        const canvas = canvasRef.current;
        if (!canvas)
            return;
        const ctx = canvas.getContext('2d');
    ...
    // Crop
    // ---------------------------------------------------------------------------
    const setCropRegion = useCallback((region) => {
    ...
    // Resize
    // ---------------------------------------------------------------------------
    const applyResize = useCallback((width, height) => {
        const canvas = canvasRef.current;
        if (!canvas)
            return;
        const ctx = canvas.getContext('2d');
    ...
    const applyEffect = useCallback((effect) => {
        ...
        const filter = intensity !== undefined
            ? {
                type: effect,
                intensity,
            }
    ...
    // Export
    // ---------------------------------------------------------------------------
    const exportDataUri = useCallback(() => {
        const canvas = canvasRef.current;
        if (!canvas)
            return null;
