...
import { setFontFamilySymbols, setSiglumListFromFile, setSiglumSetupDialogVisible } from "../provider"
...
export const SetupDialogs = () => {
    ...
    const handleImportSiglum = useCallback(async () => {
        ...
        const hasSameSiglum = importedSiglum.some(item => siglumList.some(siglum => siglum.siglum.value === item.siglum.value))

        // @TODO: manage the duplicate of symbols when importing a siglum list
        if (hasSameSiglum) {
            window.system.showMessageBox(`The Siglum ${importedSiglum[0].siglum.value} is already present in the document. Do you want to replace it?`)
