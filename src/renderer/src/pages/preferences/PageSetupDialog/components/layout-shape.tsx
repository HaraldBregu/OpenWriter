...
import { LayoutType } from "@/pages/preferences/store/layout/layout.state";
...
const TextPortion = ({
  ...
  <>
    {showDetails && (
      ...
      >
        <p className={`${additionalClassName}`}>{apparatus.title}</p>
      </div>
    )}
    <div className="grow flex gap-2 h-full">
      {showDetails &&
        columns.map((j) => (
          <div
            key={j}
            className="w-full h-full grow border-2 border-secondary-90 border-black"
          />
        ))}
    </div>
  </>
  ...
  return (
    <div className={layoutShapeClass}>
      {apparatusDetails.length > 0 ? (
        <>
          <div className="relative bg-primary-95 w-full grow c-text-container">
            <TextPortion
            ...
            {apparatusDetails.map((apparatus) => (
              ...
              >
                <TextPortion
                  ...
                  apparatus={apparatus}
                  additionalClassName={
                    apparatusDetails.length > 3 && layoutName === 'vertical-horizontal'
                      ? "transform rotate-90 whitespace-nowrap overflow-visible"
                      : "transform rotate-0"
                  }
