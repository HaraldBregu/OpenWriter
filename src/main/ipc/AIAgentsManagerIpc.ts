/**
 * AIAgentsManagerIpc — IPC module for the named agent registry and AIAgentsManager.
 *
 * Channels (invoke/handle):
 *  - agent:list-agents      (query)   -- List all registered named agents. Returns AgentDefinitionInfo[].
 *  - agent:get-status       (query)   -- Return AIAgentsManager runtime status. Returns AgentManagerStatus.
 ...
 *
 * Streaming events are pushed from AIAgentsManager via EventBus on the
 * 'AIAgentsManager:event' channel (AgentChannels.event). The renderer subscribes
 ...
 *  - All IPC inputs are validated before being forwarded to AIAgentsManager.
 */
...
export class AIAgentsManagerIpc implements IpcModule {
  ...
  register(container: ServiceContainer, _eventBus: EventBus): void {
    const agentManager = container.get<AIAgentsManager>('AIAgentsManager')
    const agentRegistry = container.get<AIAgentsRegistry>('AIAgentsRegistry')
    ...
    /**
     * Return a snapshot of AIAgentsManager runtime metrics.
     */
    ...
    registerCommand(
      ...
      (agentId: string, providerId: string, overrides?: Partial<AgentSessionConfig>) => {
        ...
        if (!def) {
          throw new Error(`[AIAgentsManagerIpc] Unknown agent id: "${agentId}"`)
        }
     ...
     * 'AIAgentsManager:event' channel (AgentChannels.event).
     */
