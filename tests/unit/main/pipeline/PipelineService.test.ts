...
class MockAgent implements Agent {
  ...
  async *run(_input: AgentInput, runId: string, signal: AbortSignal): AsyncGenerator<AgentEvent> {
...
describe('PipelineService', () => {
  ...
  describe('start', () => {
    it('should throw when agent is not registered', async () => {
      await expect(service.start('nonexistent', { prompt: 'test' })).rejects.toThrow(
        '[PipelineService] Unknown agent "nonexistent"'
      );
    });
  ...
  describe('isRunning', () => {
    ...
    it('should return false after run completes', async () => {
      const agent = new MockAgent('test', [{ type: 'done', data: { runId: '' } }]);
      registry.register(agent);
