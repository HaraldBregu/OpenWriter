...
const CustomizeToolbarSection: React.FC<{ className?: string, section: string; buttons: { className?: string, enabled: boolean; label: string; icon: any; action: string }[]; selectedItems: string[]; onButtonClick: (action: string) => void }> = ({ className = 'justify-start', section, buttons, selectedItems, onButtonClick }) => {
    return (
        <div key={section} className={cn("flex flex-wrap items-start gap-2 border border-grey-80 rounded-md p-2", className)}>
            ...
            {buttons.map(({
                ...
                <Button
                    key={action}
                    disabled={!enabled}
                    className={cn("text-grey-10 flex flex-col font-normal p-1 border-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 h-auto break-words whitespace-normal text-center", className)}
                ...
                >
                    {Icon && <Icon intent="secondary" inheritColor={true} variant={enabled ? 'tonal' : 'outline'} />}
                    {label}
                </Button>
            ))}
        </div>
...
const CustomizeToolbarModal: React.FC<CustomizeToolbarModalProps> = ({ existingToolbarItems, isOpen, onCancel, onSaveToolbarOptions }) => {
    ...
    const toolbarData: any = [
        ...
        },
        {
            ...
            buttons: [
                ...
                { enabled: true, label: t("toolbar.readingType"), icon: null, action: "readingType" },
                { enabled: true, label: t("toolbar.readingSeperator"), icon: ReadingSeperator, action: "readingSeparator" },
            ],
        },
        {
            section: "insert",
    ...
    return (
        <Modal
            ...
            className="max-w-[904px] h-auto max-h-[90%] gap-0"
            contentClassName="flex flex-col gap-8 p-8"
            footerClassName='h-[auto] pt-4 pb-4'
            showCloseIcon={true}
            actions={[
...
export default CustomizeToolbarModal;