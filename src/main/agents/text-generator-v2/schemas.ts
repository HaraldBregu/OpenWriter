...
export const INTENT_VALUES: readonly TextGeneratorV2Intent[] = [
	'continue',
	'edit',
	'rewrite',
	'summarize',
...
export const TARGET_VALUES: readonly TextGeneratorV2Target[] = ['selection', 'full'];

export const INTENT_FORMAT = {
	type: 'json_schema',
	name: 'text_generator_v2_intent',
	strict: true,
	schema: {
		type: 'object',
...
export function parseIntentClassification(raw: string): TextGeneratorV2IntentClassification {
	const parsed = parseJson(raw);

	const intent = parsed.intent;
	if (typeof intent !== 'string' || !INTENT_VALUES.includes(intent as TextGeneratorV2Intent)) {
		throw new Error(`parseIntentClassification: invalid intent ${JSON.stringify(intent)}`);
	}

	const target = parsed.target;
	if (typeof target !== 'string' || !TARGET_VALUES.includes(target as TextGeneratorV2Target)) {
	...
	return {
		intent: intent as TextGeneratorV2Intent,
		target: target as TextGeneratorV2Target,
		operation,
		...(style ? { style } : {}),
	};
}
