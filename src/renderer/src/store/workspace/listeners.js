...
import { loadResources, loadDocuments, loadIndexingInfo, loadProjectName, selectWorkspace, createWorkspace, } from './actions';
/**
 ...
 */
startAppListening({
    matcher: isAnyOf(handleWorkspaceChanged, selectWorkspace.fulfilled, createWorkspace.fulfilled),
    effect: async (_action, { dispatch, getState }) => {
        const currentPath = getState().workspace
            .currentPath;
        if (!currentPath)
            return;
        await Promise.all([
