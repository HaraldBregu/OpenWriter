...
export function useWorkspaceValidation() {
    ...
    useEffect(() => {
        const unsubscribe = window.workspace.onDeleted((event) => {
            console.warn('[useWorkspaceValidation] Workspace deleted:', event.deletedPath, 'reason:', event.reason);
 ...
 */
export function useWorkspaceDeletionReason() {
 ...
 */
export function useClearDeletionReason() {
    const dispatch = useAppDispatch();
    return useCallback(() => {
        dispatch(clearDeletionReason());
    }, [dispatch]);
