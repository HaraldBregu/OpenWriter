...
describe('PersonalityFilesService', () => {
	...
	describe('loadOne()', () => {
		it('should throw when no workspace is selected', async () => {
			mockWorkspace.getCurrent.mockReturnValue(null);
			await expect(service.loadOne(SECTION_ID, DATE_FOLDER)).rejects.toThrow(
	...
	describe('delete()', () => {
		it('should throw when no workspace is selected', async () => {
			mockWorkspace.getCurrent.mockReturnValue(null);
			await expect(service.delete(SECTION_ID, DATE_FOLDER)).rejects.toThrow(
				'No workspace selected'
			);
		});
		...
		it('should fall back to unlinking legacy .md file when folder not found', async () => {
			...
			expect(mockUnlink).toHaveBeenCalledWith(expect.stringContaining('legacy-id.md'));
		});
	...
	describe('saveSectionConfig()', () => {
		it('should throw when no workspace is selected', async () => {
			mockWorkspace.getCurrent.mockReturnValue(null);
			await expect(service.saveSectionConfig(SECTION_ID, { provider: 'openai' })).rejects.toThrow(
				'No workspace selected'
			);
		});
