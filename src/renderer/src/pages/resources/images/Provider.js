...
export function Provider({ children }) {
    ...
    const { selected, setSelected, allChecked, someChecked, handleToggleAll, handleToggleRow } = useSelection({ filteredEntries });
    ...
    const removeEntry = useCallback((id) => removeImage(id), [removeImage]);
    ...
    const setTypeFilter = useCallback((filter) => dispatch({ type: 'SET_TYPE_FILTER', payload: filter }), []);
    const setConfirmOpen = useCallback((open) => dispatch({ type: 'SET_CONFIRM_OPEN', payload: open }), []);
    const handleOpenFileDetails = useCallback((file) => dispatch({ type: 'OPEN_FILE_DETAILS', payload: file }), []);
    const handleFileDetailsOpenChange = useCallback((open) => {
        if (!open)
    ...
    useEffect(() => {
        setSelected((current) => {
            ...
            const hasChanged = nextSelected.size !== current.size || [...current].some((id) => !entryIds.has(id));
    ...
    useEffect(() => {
        ...
        const nextFilter = type === 'image' || type === 'video' || type === 'audio'
            ? type
