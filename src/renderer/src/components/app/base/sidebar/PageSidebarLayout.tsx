...
const PAGE_SIDEBAR_LAYOUT_COOKIE_NAME = 'page_sidebar_layout_state';
const PAGE_SIDEBAR_LAYOUT_COOKIE_MAX_AGE = 60 * 60 * 24 * 7;
...
const PAGE_SIDEBAR_LAYOUT_KEYBOARD_SHORTCUT = '';

type PageSidebarLayoutContextProps = {
...
const PageSidebarLayoutContext = React.createContext<PageSidebarLayoutContextProps | null>(null);

function usePageSidebarLayout() {
	const context = React.useContext(PageSidebarLayoutContext);
	if (!context) {
		throw new Error('usePageSidebarLayout must be used within a PageSidebarLayoutProvider.');
	}

	return context;
}

function PageSidebarLayoutProvider({
	defaultOpen = true,
	open: openProp,
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			document.cookie = `${PAGE_SIDEBAR_LAYOUT_COOKIE_NAME}=${openState}; path=/; max-age=${PAGE_SIDEBAR_LAYOUT_COOKIE_MAX_AGE}`;
		},
		[setOpenProp, open]
	);
	...
	React.useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			if (
				event.key === PAGE_SIDEBAR_LAYOUT_KEYBOARD_SHORTCUT &&
	...
	const contextValue = React.useMemo<PageSidebarLayoutContextProps>(
		() => ({
			state,
			open,
			setOpen,
	...
	return (
		<PageSidebarLayoutContext.Provider value={contextValue}>
			<div
		...
		</PageSidebarLayoutContext.Provider>
	);
}
...
function PageSidebarLayout({
	...
	const { isMobile, state, openMobile, setOpenMobile } = usePageSidebarLayout();
...
function PageSidebarLayoutTrigger({
	...
	const { toggleSidebar } = usePageSidebarLayout();
...
function PageSidebarLayoutMenuButton({
	...
	const { isMobile, state } = usePageSidebarLayout();
