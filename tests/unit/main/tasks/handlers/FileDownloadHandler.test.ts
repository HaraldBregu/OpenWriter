...
describe('FileDownloadHandler', () => {
	...
	describe('validate', () => {
		it('should throw when url is missing', () => {
			expect(() => handler.validate({} as FileDownloadInput)).toThrow('Download URL is required');
		});
		...
		it('should throw for a file:// URL (non-HTTP protocol)', () => {
			expect(() => handler.validate({ url: 'file:///etc/passwd' })).toThrow(/Unsafe protocol/);
		});
		...
		it('should throw for 192.168.x.x private range URLs', () => {
			expect(() => handler.validate({ url: 'http://192.168.1.100/file.zip' })).toThrow(
				'Downloads from private networks are not allowed'
			);
		});

		it('should throw for 10.x.x.x private range URLs', () => {
			expect(() => handler.validate({ url: 'http://10.0.0.1/file.zip' })).toThrow(
				'Downloads from private networks are not allowed'
			);
		});

		it('should throw for 172.16.x.x private range URLs', () => {
			expect(() => handler.validate({ url: 'http://172.16.0.1/file.zip' })).toThrow(
				'Downloads from private networks are not allowed'
			);
		});

		it('should not throw for a valid public HTTPS URL', () => {
			expect(() => handler.validate({ url: 'https://example.com/file.zip' })).not.toThrow();
		});

		it('should not throw for a valid public HTTP URL', () => {
			expect(() => handler.validate({ url: 'http://example.com/file.pdf' })).not.toThrow();
		});
	...
	describe('execute — pre-start cancellation', () => {
		it('should throw "Download cancelled" when signal is already aborted', async () => {
			...
			await expect(
				handler.execute({ url: 'https://example.com/file.zip' }, controller.signal, makeReporter())
	...
	describe('SSRF protection — edge cases', () => {
		...
		it('should block the link-local range 169.254.x.x', () => {
			expect(() => handler.validate({ url: 'http://169.254.169.254/latest/meta-data/' })).toThrow(
				'Downloads from private networks are not allowed'
			);
		});
	});
