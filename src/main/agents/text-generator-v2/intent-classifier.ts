...
import type { TextGeneratorV2IntentClassification, TextGeneratorV2ParsedInput } from './types';
...
export async function classifyIntent(
	...
	signal: AbortSignal
): Promise<TextGeneratorV2IntentClassification> {
...
function buildUserMessage(parsed: TextGeneratorV2ParsedInput): string {
	const lines: string[] = [];
...
function extractOutputText(response: unknown): string {
	const record = response as { output_text?: string; output?: Array<Record<string, unknown>> };
