...
import type { TasksManagerExecutor } from '../tasksManager/TasksManagerExecutor'
import type { TaskOptions, ActiveTask } from '../tasksManager/TasksManagerDescriptor'
 ...
 *
 * Streaming events are pushed from TasksManagerExecutor via EventBus on the
 ...
 */
export class TasksManagerIpc implements IpcModule {
  readonly name = 'task'

  register(container: ServiceContainer, eventBus: EventBus): void {
    const executor = container.get<TasksManagerExecutor>('taskExecutor')
    ...
    eventBus.on('window:closed', (event) => {
      ...
      if (cancelledCount > 0) {
        console.log(`[TasksManagerIpc] Cancelled ${cancelledCount} task(s) for closed window ${windowId}`)
      }
    })
