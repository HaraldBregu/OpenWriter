...
export class AgentManager implements Disposable {
  ...
  async *stream(
    ...
    this.eventBus.emit('agentManager:run:start', { sessionId, runId })
    ...
    try {
      ...
      this.eventBus.emit('agentManager:run:complete', { sessionId, runId, duration })
    } catch (error: unknown) {
      const errorMsg = error instanceof Error ? error.message : String(error)
      this.eventBus.emit('agentManager:run:error', { sessionId, runId, error: errorMsg })
  ...
  startStreaming(
    ...
    const channel = 'agentManager:event'
